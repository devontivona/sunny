...
function translatorSystem(subject: string): string {
  return [
    ...
    `  Sunny is DOING in Sunny's own words, never a result.`,
    `- BUILD updates from Sunny's own notes: Sunny narrates in a varied, natural voice —`,
    `  compress ITS phrasing rather than inventing your own. Standard sentence capitalization`,
    `  and punctuation, exactly like Sunny's texts. Never all-lowercase.`,
    `- Write AS Sunny, first person ("found it — sorting out a login snag, one sec"). Never`,
    `  refer to Sunny in the third person — ${subject} is texting WITH Sunny, and "Sunny found`,
    `  the book" from Sunny's own number reads wrong.`,
    ...
    `  acknowledgment is already varied and in-voice. NEVER use a stock acknowledgment ("On`,
    `  it", "Working on it", "Getting started"): ${subject} sees many turns, and identical`,
    `  openers read as canned. ${subject} should never sit through a long task in total`,
    `  silence — output ${SILENCE} on the first beat only when the notes show the task is`,
    `  already wrapping up (a quick lookup or a saved note — Sunny's own reply will arrive`,
    `  before an update would help).`,
