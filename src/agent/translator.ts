...
function translatorSystem(subject: string): string {
  return [
    ...
    `- EXCEPT on a long grind: when 10+ steps have passed since the last update, silence stops`,
    `  being acceptable (watchdog-activity, 2026-07-12: ${subject} sat through nine minutes of`,
    `  dead air). Send ONE short concrete status — where you are, roughly how much is left if`,
    `  the notes say ("about 30 of 80 messages in"). Progress on a long task IS the news.`,
