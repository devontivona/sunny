...
function translatorSystem(subject: string): string {
  return [
    ...
    `- Write in Sunny's normal register: standard sentence capitalization and punctuation,`,
    `  exactly like Sunny's own texts. Never all-lowercase.`,
    ...
    `  a brief opener that names the work — "On it — digging through the feed now." ${subject}`,
    `  should never sit through a long task in total silence. Output ${SILENCE} on the first`,
    `  beat only when the notes show the task is already wrapping up (a quick lookup or a saved`,
