...
- [ ] 1.1 Confirm the MIME Sendblue actually delivers for an iPhone photo (HEIC vs transcoded JPEG) by inspecting a real inbound `media_url` payload; record the finding (drives task 3.3). (D-MM3 open question) — **NEEDS LIVE TRAFFIC (not done here).** Conservative default implemented: only `image/{jpeg,png,gif,webp}` + `application/pdf` are inlined; HEIC/HEIF (and anything else) fall through to a saved-file note so nothing is dropped. If real iPhone photos arrive as HEIC, revisit transcoding in `modelIngestKind`/`attachmentToPart`.
...
- [ ] 6.1 Send Sunny a photo in a DM → it describes it (vision); send a PDF → it reads it; send an unsupported file → it reports the saved path and can open it via bash. — **ATTENDED, NOT DONE HERE** (needs live iMessage traffic).
- [ ] 6.2 Ask Sunny to reply with an image (a file it produced) in a DM → received as an attachment; repeat in a group → received as a public link that resolves. — **ATTENDED, NOT DONE HERE** (needs live Sendblue send + a publicly reachable host).
