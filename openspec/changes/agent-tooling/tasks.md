...
- [x] 6b Request-a-credential capability: `credential_manage` tool (owner-DM) — `list` credentials, `register` a name→reference and **test-resolve to verify** it points at a real value without surfacing it; the tool/prompt direct Sunny to ask the owner (via send_message) for a missing credential rather than invent a reference (D-CR5). `src/agent/tools/credentialManage.ts`. *(Unit-tested.)*
