...
- Add a **deterministic test harness** (Vitest) with two lanes: **unit** (pure logic) and **integration** (real modules against ephemeral Postgres), and **write the initial coverage** for the existing surface — not just scaffolding.
- Establish **test seams/fakes** so neither lane calls a paid or external service: a **mock language model** (AI SDK `MockLanguageModelV3` from `ai/test` — scripts deterministic tool calls / text), a **fake Gateway driver** (captures outbound, injects inbound — no Sendblue), and an **ephemeral Postgres** fixture. Time-dependent code is tested via **Vitest fake timers** (no production clock-injection refactor); a small refactor extracts a few pure helpers out of `runTurn` so the D-MG8 delivery logic is unit-testable directly.
...
### New Capabilities
- `automated-testing`: Deterministic unit + integration test harness, the seams/fakes (mock model, fake gateway, ephemeral Postgres, fake-timer time control) that make the agent testable without paid/external calls, the **initial written coverage** of the current surface, and the CI gate.
...
- **New dev dependencies**: `vitest` (+ coverage), `vitest-evals`, `autoevals`, an ephemeral-Postgres mechanism (Testcontainers or a disposable Docker pg), and a loader for eval case files. Optionally Mastra/agentevals trajectory scorers. No new production dependencies; AI SDK's `ai/test` mocks are already available via `ai`.
- **New directories / scripts**: colocated `*.unit.test.ts` / `*.integration.test.ts`; an `evals/` tree (cases, graders, harness); npm scripts (`test`, `test:integration`, `eval`) and a GitHub Actions workflow with a Postgres service container.
