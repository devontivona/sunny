...
- **New dev dependencies**: `vitest` (+ coverage), `vitest-evals`, `autoevals`, `@electric-sql/pglite` (in-process Postgres — no Docker), and a loader for eval case files. Optionally Mastra/agentevals trajectory scorers. No new production dependencies; AI SDK's `ai/test` mocks are already available via `ai`.
