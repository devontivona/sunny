...
- [ ] 5.3b Containment boundary: run untrusted-derived commands in a sandbox (bubblewrap/Landlock/seccomp) with **restricted egress**; egress control treated as load-bearing (tool-access R: containment; security; R13 + lethal-trifecta note). *(Open: sandbox-untrusted-only vs sandbox-all-bash — Devon's call.)*
