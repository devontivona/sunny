...
- [ ] 4.1 Idempotent conversational turns keyed by message id; re-process un-answered messages on restart; inbound dedup; **serialize turns per thread** so rapid messages queue rather than race (durable-execution R: idempotent turns, D-DE1; R7).
...
- [ ] 5.3 Tool registry: each tool declares risk tier + allowed `op://` refs; runtime enforces gating + reference whitelist; conservative defaults (tool-access R: tiers, whitelist, defaults; D-TA1/2, D-CR3).
- [ ] 5.4 Initial tools: read-only shell, web search/fetch, file read, **build/run sites via the `devbox` skill** (preview/share URLs; durable public deploy approval-gated/deferred), **email via Sunny's own mailbox `sunny@waywardlane.com` using the himalaya CLI** (CC/forward to act; send approval-gated; bodies untrusted → injection-contained, ideally a no-credential subagent), todos — each tiered per D-TA2 (R3, R5).
