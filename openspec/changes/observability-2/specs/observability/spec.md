...
Sunny SHALL meter token usage and cost per run and over rolling windows, and SHALL enforce configured caps (including the per-run cost cap and autonomous rate limit used by scheduling). A run that exceeds its cap SHALL be stopped and the user notified rather than continuing to spend. Enforcement SHALL be performed in the agent loop and SHALL NOT depend on Langfuse; Langfuse provides cost/usage reporting and visualization.
...
Sunny SHALL be able to produce a concise human-readable insights summary (token usage, cost, tool breakdown, activity) and deliver it over the messaging gateway on request or on a schedule. Detailed, interactive exploration of this data SHALL be provided by Langfuse's dashboards rather than re-implemented in Sunny.
