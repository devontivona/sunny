...
**Migration**: Structural silence is expressed by the `nobody` audience (record-only). Per-run conditional silence is expressed by the lane's silence sentinel (see the ADDED voice-layer requirement). A run's result is still always recorded for inspection regardless of silence.
