...
- [ ] 1.3 Re-wire `ensureConsolidationSchedule` (currently defined-but-never-called) so fresh installs seed nightly consolidation, as a `household(silent)` singleton over the shared core — drop the "review the recent conversation" framing (D-RA2, D-RA10).
...
- [ ] 2.2 Replace the `user`/`parent`/`silent` **output target** with an Audience end-to-end (`emitStep`, the four workflow entrypoints, `src/agent/outputTarget.ts`); `silent`→`household(silent)`, `parent`→`parent(...)` (D-RA2) — modifies durable-execution "Configurable output target".
...
## Verify
- [ ] 4.1 Scenario coverage from `design.md`: the Leo reminder (person, conditional send via empty reply, delivers to Kate), the follow-up sweep (household fan-out to multiple members), consolidation (household silent, records only), a subagent (parent audience, authority ⊂ parent), and ownership (Kate lists/cancels her runs; owner sees all).
