...
Every durable run SHALL be addressed by an **Audience** (the run-audiences capability) — `person`, `household`, `thread`, or `parent` — rather than a fixed `user`/`parent`/`silent` output target. A run SHALL emit only by invoking a messaging tool, whose destination is resolved from its Audience; a run not endowed a messaging grant SHALL send no proactive message and SHALL still record its result (silence is structural, not an output mode). A `parent` run's messages SHALL be delivered to its spawning run.
...
- **WHEN** a run with a `person` audience sends its result
- **THEN** the message is delivered to that person's conversation via the gateway, even if that person is not the owner
