...
- **run-audiences (new)** — the model: an **Audience** (logical recipient: `person` / `household` / `thread` / `parent`) that resolves to a **Thread** (physical durable log with an optional channel binding: `bound` | `detached`); a **Principal** (the subject a run acts for → who owns it); an **authority** set governed by **monotone subset attenuation** (a spawned run's authority ⊆ its creator's, endowed explicitly at spawn, no ambient authority; the spawn relation is a **derivation tree**); and a single **RunSpec** `{ audience, authority, brief, model }` that conversation / job / schedule / subagent all share.
- **scheduling (modified)** — restore self-scheduling (family-gated, audience-aware); re-seed nightly consolidation as a `household(silent)` singleton; reframe the anti-recursion guard as authority attenuation; deliver by resolving the schedule's audience, not a hardcoded owner thread.
- **durable-execution (modified)** — replace the `user`/`parent`/`silent` **output target** with an **Audience**; generalize **least-privilege child runs** into **authority attenuation for every spawned run** (not just delegated children).
- **tool-access (modified)** — the three creation verbs (`start_job`, `delegate_task`, `schedule_create`) share one `{ audience, authority }` argument shape; unify inspection/lifecycle into audience-agnostic `list_runs` / `cancel_run`; authority is endowed explicitly at spawn and enforced by subset-`tools` construction + `activeTools`.
