...
The runtime SHALL idempotently seed the dreaming schedule (label `dreaming`): cron every 4 hours, silent/household output (result recorded, nothing sent), authority `memory_read, memory_write, bash, file_read` (never the spawn grants), and a prompt that directs the run to follow the dreaming skill. Seeding SHALL retire the legacy `nightly-consolidation` schedule row. The fired run uses the standard scheduled-run engine — no dedicated workflow or dispatch branch.
