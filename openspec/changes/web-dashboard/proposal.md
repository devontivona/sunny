...
Add a **`web-dashboard`** capability: a server-rendered, read-only, terminal-UI-styled dashboard served by the existing Nitro app, reading from the memory files + Postgres (no new datastore). Pages: **SUNNY.md**, **USER.md**, a **memory browser** (INDEX + topic docs), **conversation** (recent messages + Sunny's retained scratch + keyword search), **schedules & runs**, and **activity & health**. Access is gated by an **iMessage-approval device-pairing** flow: an unknown device triggers Sunny to DM the owner for approval; on approval the device receives a signed token cookie.
...
- **New Nitro routes** under `server/routes/dashboard/` plus read-only queries over the existing memory files and Postgres tables. No schema changes except a small **sessions/access-request** store for auth.
