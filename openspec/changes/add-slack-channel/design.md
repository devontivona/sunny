...
The existing `Authorizer` resolves `senderId` against roster identities; Slack member IDs are added to `config.owner.identities` as data — no schema change (identities were spec'd as channel-stable addresses). Implementation note: `normalize()`'s phone branch originally fired for ANY digit-bearing non-email string, which would have collapsed `U0123ABC` to `+0123` (colliding all Slack identities); it now applies only to phone-shaped strings (digits + phone punctuation), with a regression test. v1 posture: DMs from the owner's Slack ID dispatch; DMs from anyone else are dropped (fail-closed, logged); mention/channel events are received (handlers registered, manifest scopes granted) but never dispatched because participants aren't rostered. Future group support becomes roster data + a trust-policy decision, not rearchitecture.
