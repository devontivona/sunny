...
**D4 — Langfuse fix is a distinct workstream; source-confirmed it does NOT come free with v7.** Root cause (verified in source, not assumed): the `workflow` runtime re-runs the orchestration body on every resume — `@workflow/core` `runtime.js` wraps each resume in `trace('workflow.replay', …)` with **no `isReplaying` gate**. The `ai.streamText.doStream` generation span lives *inside* the journaled `doStreamStep` `'use step'`, so it is **replay-safe** (our `doStream` count was correctly 2); but the `ai.streamText` wrapper + `ai.toolCall` spans are emitted **from the agent loop, around the cached step**, so they re-emit per resume (our 5× and 8×). **v7 WorkflowAgent keeps the same placement:** telemetry moved to a callback dispatcher (`createRestrictedTelemetryDispatcher`), but the dispatch calls fire from the same `while(!done)` loop around the cached step (`@ai-sdk/workflow@1.0.4`), so replay still re-fires them — and the bridge is explicitly provisional (`vercel/ai` #12164, whose telemetry TODOs are NOT replay-dedup). We are **not** doing anything unique: our LiveBus `getReadable()` reads the output chunk stream (not telemetry), and the custom span processors each run once; the Langfuse Vercel-AI-SDK integration is generic OTel ingestion with no replay/dedup awareness, so the fix MUST be at the emission side. Plan: (1) **after migrating, drive one turn and inspect the trace** to confirm the counts empirically — the only thing not already source-proven; (2) the fix is an OTel `SpanProcessor`/`Sampler` that **drops spans created within a `workflow.replay` context** (the wrapping span is the detect flag), re-hosted under `@ai-sdk/otel`/`registerTelemetry` (where `TracePromotingSpanProcessor` + `RedactingSpanProcessor` must move anyway); (3) **report upstream** on `vercel/ai` (ref #12164) — the callback dispatcher is the natural place for an `isReplaying`-aware guard. *Alternative — assume v7 fixes it:* rejected; source predicts persistence with high confidence.
...
## Spike results (D7 gate) — task 1.1, confirmed 2026-06-27

Ran an isolated `@ai-sdk/workflow@1.0.4` + `ai@7.0.4` `WorkflowAgent` workflow on the DevKit runtime (scratch harness `scratchpad/v7-spike/`, matching repo runtime versions `workflow@4.5.0` / `@workflow/world-postgres@4.2.0` / `@workflow/vitest@4.0.11`).

- **Interop CONFIRMED.** On the `@workflow/vitest` Local World, the `WorkflowAgent` workflow executes, journals its model-call step (`doStreamStep`) + tool step, and durably replays after a forced post-agent crash with EXACTLY-ONCE side effects (replay drove 2 finalize attempts while the journaled tool step ran once). The WDK `StandaloneBuilder` compiled `WorkflowAgent` into deployable bundles (manifest: 13 steps, 1 workflow, 3 classes), and `@workflow/world-postgres` provisioned its schema cleanly with the v7 deps.
- **Direct `@ai-sdk/anthropic` instance works natively — NO gateway, NO shim (CONFIRMED with a live call).** Passed `anthropic('claude-haiku-4-5')` as a real instance to `new WorkflowAgent({ model })`; it serialized across the journaled `doStreamStep` boundary, ran a real turn (model replied "Hi"), completed, and replayed after a forced post-agent crash with only ONE model call (doStreamStep journaled/memoized). This works because the v7 AI-SDK providers implement the WDK class-serialization protocol: `AnthropicLanguageModel` has `static [WORKFLOW_SERIALIZE]/[WORKFLOW_DESERIALIZE]` imported from `@workflow/serde` — the SAME symbols `@workflow/core`'s serializer dispatches on — and the WDK builder auto-registers the model class (the manifest's "3 classes" were `@ai-sdk/gateway`'s registered model classes when a string was used). So `conversation.ts`/`buildTurnModel` can keep passing a direct `@ai-sdk/anthropic` instance to `WorkflowAgent` exactly like it did to `DurableAgent`. Prompt caching / adaptive thinking via `providerOptions` are unaffected.
- **Standalone real-Postgres run: tooling-blocked, not interop-blocked.** `workflow dev` is "coming soon" and CLI `start` requires a nitro-style deployment in the world, so a standalone one-shot Postgres run isn't available without reimplementing the nitro runtime glue. The Postgres world is the SAME `@workflow/world` interface the Local World implements (serialization is done in `@workflow/core`, world-agnostic), so residual Postgres-backend risk is low and is covered by the repo's existing `@workflow/vitest` suite + loopback live smoke after cutover (tasks 3.5 / 8.1).

**D2-bis — the only model gotcha is the TEST mock, not production (corrected; earlier "needs a gateway/shim" was WRONG).** Production passes a direct `@ai-sdk/anthropic` instance (WDK-serializable, see above) — no change from the v6 pattern, no Vercel gateway. The real constraint is the **test seam (task 3.4):** `ai`'s `MockLanguageModelV4` does NOT implement `@workflow/serde`'s `WORKFLOW_SERIALIZE`/`WORKFLOW_DESERIALIZE`, so passing a mock INSTANCE as the model fails step serialization (`.args[1]`). Fix options for the mock: (a) implement the `WORKFLOW_SERIALIZE`/`WORKFLOW_DESERIALIZE` statics + a `classId` on a small custom mock model so the WDK serializer can round-trip it; (b) keep threading mock responses as serializable DATA (the current `turnModel.ts` already passes `testModelResponses` as plain data read in the `setupTurn` step) and build the mock INSIDE the model step; or (c) resolve a mock model-id via a registered serializable class. Pick whichever keeps the deterministic-turn harness simplest (the spike used an in-step `gateway.languageModel('mock/..')` patch as a quick seam, but production has no such need).

**Other spike notes for later tasks:**
- The v7 mock model class is `MockLanguageModelV4` (from `ai/test`), NOT `MockLanguageModelV3` as task 3.4 / D6 say — update that reference. (And it is NOT WDK-serializable — see D2-bis.)
- `result.toolResults` came back EMPTY at the top level even though the tool executed (verified via a side-effect counter). The delivery classification in `finalizeTurn` reads tool calls/results — under v7 source them from `result.steps[].content` / `result.messages`, not the top-level `toolResults` aggregate. Relevant to task 3.2.
- Known SWC-plugin quirk (vercel/workflow#1365): `new ClassName(...)` used inside a step closure needs the constructor wrapped in a function imported from a SEPARATE file (the package does this for its mock). Applies to building the anthropic/recovery model instances inside steps.
- Step bundles run in a SEPARATE module context from the orchestration body; module-level globals are NOT shared across the boundary (the repo's workflow harness already bridges via a `globalThis` symbol — keep that pattern).

## Open Questions

- Does `@ai-sdk/workflow` `WorkflowAgent` expose a **safe in-flight steering / message-queue** primitive, or does `loadSteers` stay?
- ~~Does `WorkflowAgent` still re-emit per-step spans on replay?~~ **Answered (source): yes** — same dispatcher placement around the cached step (see D4). The only open part is confirming the **exact span counts** on our runtime post-migration by inspecting one trace.
- ~~Does `@workflow/world-postgres` run `@ai-sdk/workflow` workflows without issue (D7 spike)?~~ **Answered (spike): yes at the runtime level** (Local World replay test + StandaloneBuilder compile + Postgres schema provisioning); standalone Postgres execution deferred to the repo harness post-cutover (tooling-blocked). See "Spike results".
