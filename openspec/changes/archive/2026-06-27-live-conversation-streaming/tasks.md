...
- [x] 9.4 Manual check on the devbox — VERIFIED via live testing on the worktree devbox (2026-06-27): live turn streaming (auto-stick-to-bottom, no horizontal scroll, settles to persisted), home "active now" indicator + deep links, per-step rendering with the JSON drawer, recovered-turn `[R]`, and background-job streaming + historical view (shared `LivePane`). Remaining post-merge op: repoint devbox off the worktree to main and restart (HMR can serve stale code) — see [[sunny-devbox-worktree]].
