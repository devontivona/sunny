# builtin-surface Specification

## Purpose
The builtin surface is the git-committed `agent/` directory in the project repository that holds all developer-authored runtime artifacts (skills, schedules, memory starters, default config), organized by seeding mechanism: `agent/builtin/` artifacts are read in place and stay developer-authoritative for their lifetime, updating with every code deploy, while `agent/seeds/` templates are materialized write-if-missing exactly once and then owned by the runtime. This gives shipped agent content a single discoverable home in the repo — machine-agnostic and free of inline string literals in source — with a clear ownership boundary between what a deploy controls and what the running agent controls.
