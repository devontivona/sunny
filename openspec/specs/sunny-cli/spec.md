# sunny-cli Specification

## Purpose
The generic, repo-owned self-interaction CLI: capabilities that operate on Sunny's own state ship as tested subcommands invoked over bash and documented by skills, rather than as new native tools. Anything a readonly (bash-less) run needs stays a native tool.
